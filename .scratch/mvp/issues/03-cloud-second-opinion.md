...
已交付：`src/background/service-worker.js`（importScripts 加载共享模块，同步注册 onMessage 监听，async 响应 + return true）+ `src/cloud/second-opinion.js`（按回答 ID 缓存 LRU 500、按 tab 每页预算、并发 ≤2、30s 超时、失败返回 null 降级）。
