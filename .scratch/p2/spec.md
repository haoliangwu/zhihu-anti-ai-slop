...
| # | 任务 | 优先级 | 状态 |
|---|---|---|---|
| 01 | SPA 设置页（自定义正则规则 + judge prompt + 全量配置） | **P0** | resolved |
| 02 | AI 判定 → 隐藏正文 + 直接渲染原因/证据（折叠过滤二合一） | **P1** | resolved |
| 03 | 专栏文章页支持 | done（由 `.scratch/articles/` 功能实现） | 曾被暂缓，articles 功能落地后已实现 |
| 04 | 自定义 API 域名动态权限 | **P2** | resolved |
| 05 | 本页判定统计面板 | backlog | 用户确认优先级低 |
| 06 | 判定/覆盖数据导出 | backlog | 用户确认优先级低 |

> P2 期间用户追加（已随 01/02 交付，不属于原始任务清单）：回答卡片"重新判定"按钮（强制绕过二审缓存重跑）、设置页"清除二审缓存"按钮。
...
- **自定义正则规则**：增删改 AI 痕迹正则（名称/正则/权重/命中上限），与内置 21 类合并参与初审；无效正则即时校验
...
确定/疑似 AI 回答**直接隐藏正文**，角标处渲染原因（痕迹清单）与证据（二审依据），提供"展开原文"按钮。覆盖为"认为人工"的不受影响；SPA 重渲染后自动重新应用。
> 面板展开策略（用户后决定，覆盖最初"默认内联展示"）：所有面板（含二审）统一默认收起，点击角标展开/收起，避免遮挡正文。详见 `issues/02-ai-hide-body.md`。
