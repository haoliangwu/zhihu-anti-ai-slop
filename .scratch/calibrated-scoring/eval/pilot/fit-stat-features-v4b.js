...
async function main(){
  const humans=await loadDataset(1),oldAi=await loadDataset(0),flash=await loadFlash();
  const seeds=[20260828,1,42,7,123,999,5555];
  const wsum=new Array(D).fill(0);let bsum=0;let lastFit=null;
  ...
  const w=wsum.map(v=>v/seeds.length);const b=bsum/seeds.length;
  ...
  const buckets=(rs)=>{let c=0,s=0,n=0;for(const r of rs){const sc=scoreRow(r);if(sc<=30)c++;else if(sc<=50)s++;else n++;}return{c,s,n:rs.length};};
  const bh=buckets(humans),bf=buckets(flash),bo=buckets(oldAi);
  const pct=b=>(b.c/b.n*100).toFixed(1)+' / '+(b.s/b.n*100).toFixed(1)+' / '+((b.n-b.c-b.s)/b.n*100).toFixed(1);
  console.log('=== v4b (词法+4统计, 去entropy/ttr) 三方分布 确定/疑似/正常 ===');
  console.log('  human :',pct(bh));
  console.log('  flash :',pct(bf));
  console.log('  oldAI :',pct(bo));
  ...
  let ch=0,cf=0;for(let k=0;k<th.length;k++)if((predictStd([th[k]])[0]>=0.5?1:0)===th[k].label)ch++;for(let k=0;k<tf.length;k++)if((predictStd([tf[k]])[0]>=0.5?1:0)===tf[k].label)cf++;
  console.log('\ntest acc: human '+(ch/th.length).toFixed(4)+'  flash '+(cf/tf.length).toFixed(4));
  ...
  console.log('case:',Math.round(sig(b+caseX.reduce((s,v,i)=>s+w[i]*v,0))*100),'(v3:45 v4a:20)');
  console.log('\n=== stat weights ===');
  const sn=['sCV','punctDensity','sMeanLen','commaRatio'];
  for(let i=0;i<4;i++)console.log('  '+sn[i].padEnd(14),+w[D_LEX+i].toFixed(6));
  console.log('  intercept:',+b.toFixed(6));
  const fn=(bf.n-bf.c-bf.s)/bf.n;
  console.log('\nflash 漏判 v3 38.1% -> v4b '+(fn*100).toFixed(1)+'% '+(fn<0.381?'PASS':'FAIL'));
}
