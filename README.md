...
4. （可选）点击工具栏图标打开设置页：填入 API Key（OpenAI 兼容，默认 DeepSeek）并保存——首次保存会请求授权该 API 域名，授权后模糊判定的回答启用云端复核；「作者管理」卡片可按主页链接批量添加/移除屏蔽或信任作者
...
src/
  ...
  shared/storage.js            设置、覆盖、作者规则、二审缓存的存储助手
