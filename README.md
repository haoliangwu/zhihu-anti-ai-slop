...
2. **输入窗口**：只取块级正文段落（默认 ≤2000 字，跳过标题/图/引用/图片占位；等待渲染稳定后再判定；可切"只看开头一两段"）
3. **规则初审**：命中 AI 创作痕迹（套话、总分总骨架、空洞强调、冒号/引号滥用、编号式结构等 21 类）即扣分，输出规则分与命中清单
4. **云端二审**（可选）：规则分落入模糊带（默认 20–80）时，将正文发送至 OpenAI 兼容接口（默认 DeepSeek）复核；按回答正文哈希缓存、每页限次、失败自动降级
5. **角标与覆盖**：渲染判定角标（含灰色"跳过"标记短回答）；点击展开理由面板；可覆盖为"认为人工 / 认为 AI"，记录存本机
...
> 提示：扩展更新（重新加载）后，已打开的知乎页面里的旧内容脚本会失效（按钮无响应属正常现象），**刷新页面即可**。

## 配置

点击工具栏扩展图标打开设置页：

- **判定阈值**：确定 AI（默认 ≤40）/ 疑似 AI（默认 ≤70）
- **云端二审**：启用开关、API 地址、API Key（仅存本机）、模型名、每页调用上限
- **输入窗口**：字数上限、判定字数下限（少于 300 字跳过判定）、全文/开头模式
